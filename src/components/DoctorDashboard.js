...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchTotalEarnings = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/billings/total-earnings`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
    ...
    const fetchPaymentMethodsSummary = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/billings/payment-methods-summary`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
